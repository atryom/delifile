...
## Фаза 3 — Очистка фронтенда (Angular) ✅ ВЫПОЛНЕНО (10.06.2026)

- Убраны `shared_folder_only` из `FileCard`/`FileListItem`, `Folder`/`FolderTreeNode` интерфейсы
- `organization-api.service.ts` — удалены все методы локальных папок
- `domain-api.services.ts` — удалены методы папок
- `file-detail.component.ts` — убрана логика локальных папок, добавлена поддержка shared-папки через `contextSharedFolderId`
- `files-api.service.ts` — удалён `moveFolder()`; `folder_id` оставлен (нужен для shared-folder FK)
- `npm run build` — без ошибок

---

## Фаза 4 — Очистка мобильного приложения ✅ ВЫПОЛНЕНО (10.06.2026, коммит 106956c)

- Удалены `mobile/src/api/folders.ts`, `mobile/src/hooks/useFolders.ts`, `mobile/src/types/folder.ts`
- `mobile/src/types/file.ts` — убраны `shared_folder_only` и `folder_id` из `FileListParams`
- `mobile/src/api/files.ts` — удалён `moveFolder()`
- `mobile/src/hooks/useFiles.ts` — удалена мутация `useMoveFolder`
- TypeScript-сборка без ошибок

---

## Фаза 5 — Миграции БД ✅ ВЫПОЛНЕНО на ПРОДЕ (10.06.2026, коммит 6bff876)
...
**Статус:** ✅ ВЫПОЛНЕНО (10.06.2026, коммит 434804b)

Реализовано двухшаговое удаление в трёх экранах мобильного приложения:
- `mobile/src/api/shared-folders.ts` — `delete(id, force?)` добавлен параметр `force`
- `mobile/app/(app)/files/index.tsx` — обработка `FOLDER_HAS_FILES` + Alert с подтверждением
- `mobile/app/(app)/files/shared-folders/index.tsx` — аналогично
- `mobile/app/(app)/files/shared-folders/[id].tsx` — аналогично (для подпапок)
- `mobile/src/utils/error.ts` — хелпер `getFolderHasFilesCount()`
...
Исправлено:
- Бэкенд (`SharedFolderController::files()`): добавлен JOIN к таблице `files`,
  обработка `search`, `sort_by`, `sort_order`, `file_type_group`, `content_kind`, `tag_id`
- Angular (`shared-folders-api.service.ts`): все фильтры/сортировка добавлены в `listFiles()`
- Angular (`folders-tree.component.ts`): `loadSharedFolderFiles()` теперь передаёт поисковый запрос,
  сортировку, тип файлов и тег в `sfApi.listFiles()`
